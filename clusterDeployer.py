...
def _resolv_conf_parse_file(rc_file: str) -> list[str]:
    try:
    ...
    for line in rc_content.split(b'\n'):
        ...
        if not line:
            continue
        if line[0] not in (32, 9):
            continue
        addr = common.ipaddr_norm(line)
        if not addr:
            continue
...
def _resolv_conf_update(setup: bool = True) -> None:
    rc = "/etc/resolv.conf"
    rc_orig = "/etc/resolv.conf.cda-orig"
    rc_local = "/etc/resolv.conf.cda-local"
    ...
    if not setup:
        ...
        if lnk == dst:
            return
        logger.info(f"resolv.conf: restore /etc/resolv.conf to link to original {rc_orig}")
        lh.run_or_die(f"ln -snf {dst} {rc}")
        return
    ...
    if not os.path.exists(rc_orig):
        ...
        if rc_content_orig == b"":
            ...
            logger.info(f"resolv.conf: point {rc_orig} to /run/NetworkManager/resolv.conf")
            lh.run_or_die(f"ln -snf /run/NetworkManager/resolv.conf {rc_orig}")
        else:
            logger.info(f"resolv.conf: write {rc_orig}")
            with common.atomic_write(rc_orig, text=False) as f:
                f.write(rc_content_orig)

    with common.atomic_write(rc_local) as file:
        file.write(
            f"""# Written by cluster-deployment-automation.
#
# A local dnsmasq instance is configured (`systemctl status dnsmasq.service`).
# Original file is at "{rc_orig}".
#
# Set:     `sudo ln -snf {os.path.basename(rc_local)} /etc/resolv.conf`
...
"""
    ...
    if not dont_touch:
        lh.run_or_die(f"ln -snf {os.path.basename(rc_local)} {rc}")


def _dnsmasq_update(cluster_name: str, api_vip: Optional[str] = None) -> None:
    ...
    nameservers = _resolv_conf_parse_file("/etc/resolv.conf.cda-orig")
...
class ClusterDeployer:
    ...
    def update_dnsmasq(self, *, setup: bool = True) -> None:
        ...
        if setup:
            ...
            api_vip = None
        _dnsmasq_update(cluster_name, api_vip)
